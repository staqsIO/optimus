...
### Phase 1.5: Unified Content Engine

Build a content generation pipeline within Optimus that produces long-form blog posts and derives short-form LinkedIn posts from them. Blog posts are the primary artifact; LinkedIn posts are a distribution format. Content is delivered to external sites (umbadvisors.com) via git-push PRs with static MDX rendering — no runtime coupling between the content engine and consuming sites.

**Motivation:** Co-founder content execution bottleneck (conversation 012). Expanded from LinkedIn-only to unified content engine (conversation 021) after UMB Advisors blog requirement. This is the cheapest test of the generalization thesis — the voice+gate+task-graph pipeline generalizes beyond email to content generation with >70% infrastructure reuse.

**Autonomy level:** 0 — all generated content requires board member approval before publication. Blog posts are delivered as PRs requiring human merge. LinkedIn posts are Tier 3 (reputational, human-in-the-loop) via the Communication Gateway.

**Build:**
- Unified `content` schema: `content.topics` (topic queue with scheduling), `content.drafts` (with `content_type` for blog/linkedin), `content.reference_posts` (voice-matched reference corpus, embedded via pgvector)
- `executor-writer` agent — 5-phase pipeline: (1) Research (Reddit/HackerNews/web/forum polling for trending topics), (2) Grounding (RAG retrieval from knowledge base + authoritative source verification), (3) Outline + Draft (structured MDX generation with SEO metadata), (4) Image Generation (header infographic via Gemini/DALL-E), (5) Memory (agent-memory.js persistence + G11 retrospector feedback)
- `content-atomizer` agent — derives 1-3 LinkedIn-sized posts per blog post using voice guide matching. Links back to full article.
- Content-specific constitutional gates: G8 (Factual Accuracy — flags numeric claims, regulatory references, named company claims for human verification), hard constraint gates from voice guide (forbidden words, formatting bans) as `config/content-gates.json`
- Git-push adapter — creates PRs to external site repos via GitHub API (existing executor-coder pattern)
- LinkedIn poster adapter — `w_member_social` OAuth API client with clipboard fallback
- Multi-author support — voice profiles for all UMB Advisors managing partners, not just Dustin

**Content delivery topology (P2-compliant blast radius isolation):**
```
Optimus (Railway/Supabase)          umbadvisors (Vercel)
  executor-writer                     /content/blog/*.mdx
  content-atomizer                    generateStaticParams
  content gates (G7, G8)    -PR->     GitHub Action (Zod schema validation)
  budget tracking                     Board member approves PR
  audit trail                         Merge triggers Vercel rebuild
                                      Static CDN (100% available during Optimus outage)
```

**Instrument:**
- Content generation cost per post (target: <$0.50/blog post, <$0.10/LinkedIn post)
- Draft approval rate (no edits) — target: >=60% by week 2-4
- Voice gate catch rate — target: 100% of hard constraint violations caught
- Posts per week — target: 3-4 consistently
- Time per post for board member review — target: <5 min

**Phase 1.5 success metrics:**

| Metric | Target |
|--------|--------|
| Blog post generation cost | < $0.50 per post |
| LinkedIn post generation cost | < $0.10 per post |
| Draft approval rate (no edits needed) | >= 60% |
| Hard constraint gate catch rate | 100% |
| Content cadence | >= 3 posts/week |
| Board review time per post | < 5 min |
| Blog page load (static CDN) | < 200ms p99 |

**Exit criterion:** Content pipeline operational. Draft approval rate >= 60%. At least 10 blog posts published with full audit trail. G8 (Factual Accuracy) gate catching hallucinated claims. Voice match scores stable.
