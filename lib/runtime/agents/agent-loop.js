  ...
  async start(opts = {}) {
    ...
    if (this.config.llmEnabled !== false) {
      try {
        ...
        // Single-provider overlays (LLM_SINGLE_PROVIDER) remap every model to
        // one provider, which legitimately conflicts with a per-agent
        // requireProvider lock (e.g. executor-research → claudeCode). That is
        // an expected capability gap in single-key self-host mode, not a
        // misconfiguration — skip the agent instead of dying.
        const singleProvider = process.env.LLM_SINGLE_PROVIDER;
        if (singleProvider && this.config.requireProvider &&
            this.config.requireProvider !== singleProvider) {
          log.warn(
            `[${this.agentId}] Skipping: requires provider "${this.config.requireProvider}" ` +
            `which is unavailable under LLM_SINGLE_PROVIDER=${singleProvider}. ` +
            `Unset LLM_SINGLE_PROVIDER or provide that provider's credentials to enable this agent.`
          );
          if (this._tokenRefreshTimer) clearInterval(this._tokenRefreshTimer);
          this.running = false;
          return;
