...
// SSL decision for outbound Postgres connections. Managed hosts (Supabase,
// Railway proxy) need TLS; local/self-hosted Postgres (docker compose service
// hostnames like postgres:5432) speaks no TLS. An explicit sslmode=disable in
// the connection string always wins — that's the libpq contract, and it's the
// escape hatch for self-hosters whose DB hostname isn't literally "localhost".
export function sslOptionFor(connStr) {
  if (!connStr) return {};
  if (/[?&]sslmode=disable\b/.test(connStr)) return {};
