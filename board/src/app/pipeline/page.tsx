...
function PipelinePageInner() {
  ...
  useEffect(() => {
    if (!highlightTaskId || workItems.length === 0) return;
    const el = document.getElementById(`pipeline-task-${highlightTaskId}`);
    el?.scrollIntoView({ behavior: "smooth", block: "center" });
  }, [highlightTaskId, workItems]);
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {workItems.map((w) => (
                <tr
                  key={w.id}
                  id={`pipeline-task-${w.id}`}
                  className={`border-b border-white/5 ${
                    highlightTaskId === w.id ? "bg-blue-500/10 ring-1 ring-inset ring-blue-400/40" : ""
                  }`}
                >
                  <td className="p-3">
                    <span
...
export default function PipelinePage() {
  return (
    <Suspense fallback={<div className="p-5 text-sm text-zinc-500">Loading pipeline…</div>}>
      <PipelinePageInner />
    </Suspense>
  );
}
