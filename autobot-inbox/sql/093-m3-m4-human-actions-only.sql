-- Migration 093: M3 and M4 measure human-action accuracy, not system actions
--
-- M3 was reading 0% because the view counts ALL non-null board_actions in
-- inbox.drafts as "actioned" drafts. Since the auto-archive feature shipped
-- (PR #157), 119 of 119 actioned drafts in the last 14d are auto-archives
-- (board_action = 'archived_no_reply' or 'archived_external'), not human
-- approvals. SPEC §14 defines M3 as "% of drafts approved without edits" —
-- a draft *quality* signal that requires a human in the loop to be
-- meaningful. System decisions belong in a separate metric.
--
-- This migration:
--   1) Switches M3 + M4 to read from agent_graph.action_proposals (the
--      canonical action surface; the only place a human-approved draft has
--      ever been recorded — exactly 1 row at the time of writing).
--   2) Filters to board_action IN ('approved', 'edited', 'rejected') —
--      the strict human-action set. Excludes archived_* states.
--   3) Returns NULL when the sample is < 5 — "no signal" beats "0%".
--      The view consumer must treat NULL as INFO/PENDING, not FAIL.
--
-- Deferred to a follow-up (parent of this fix is STAQPRO-274; the deeper
-- product question is STAQPRO-278):
--   - M5 (drafts_reviewed_14d) currently counts auto-archives toward the
--     L0 exit threshold (≥ 50). Whether that's the right semantics is a
--     board decision, not a metric-view fix. Leaving M5 unchanged.
--   - M13 (L0 exit ready) depends on M5; left unchanged.
...
CREATE OR REPLACE VIEW agent_graph.v_phase1_metrics AS
 SELECT ( SELECT round(
    ...
    -- M3: draft accuracy (% of human-actioned drafts that were approved).
    -- Reads action_proposals (canonical), filters to human actions only,
    ...
    ( SELECT
      ...
      WHERE board_action IN ('approved', 'edited', 'rejected')
        AND acted_at >= (now() - '14 days'::interval)
    ) AS m3_draft_accuracy_pct,

    -- M4: edit rate (% of human-actioned drafts that required edits).
    -- Same source/filter as M3; same NULL-on-small-sample behavior.
    ( SELECT
        CASE
          WHEN count(*) >= 5 THEN round(
    ...
    -- M5: drafts reviewed in 14d. Unchanged — counts ALL board_action set
    -- (including auto-archives). See STAQPRO-274 / STAQPRO-278 for the
    -- pending semantics decision on whether L0 exit should require human
    ...
    -- M13: L0 autonomy exit ready. Unchanged — still depends on M5 (≥ 50)
    -- and the inbox.drafts edited rate. See STAQPRO-278 for the decision
    -- on whether human-only review counts should gate L0.
    ( SELECT
                CASE
...
DECLARE
  ...
  v_m5 BIGINT;
BEGIN
  SELECT m3_draft_accuracy_pct, m4_edit_rate_14d_pct, m5_drafts_reviewed_14d
  INTO v_m3, v_m4, v_m5
  FROM agent_graph.v_phase1_metrics;
  RAISE NOTICE '[093] m3 (was 0.00, expecting NULL with current data): %', v_m3;
  RAISE NOTICE '[093] m4 (was 0.00, expecting NULL with current data): %', v_m4;
  RAISE NOTICE '[093] m5 (unchanged): %', v_m5;
END $$;
