-- 184-match-chunks-applies-to.sql
-- ADR-017 follow-up — narrow content.match_chunks() share-grant arm to grants
-- whose applies_to includes 'documents'. Migration 183 added the column with
-- default {documents,wiki_pages}, so all existing grants continue to match
-- this filter; the column is now load-bearing only when a future grant
-- explicitly narrows applies_to (e.g., to exclude documents).
...
BEGIN
  ...
  SELECT
    ...
    -- ADR-017: shared_via provenance, restricted to grants that apply to 'documents'.
    (
  ...
  WHERE c.embedding IS NOT NULL
    ...
    AND (
      ...
      -- share-grant arm — narrowed by applies_to so a grant scoped to wiki only
      -- never accidentally exposes documents (and vice versa).
      OR EXISTS (
